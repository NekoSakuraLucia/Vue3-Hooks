 ...
 */
export function useMemo<T, D extends readonly unknown[]>(
    getter: () => T,
    dependencies: D
): ComputedRef<T> {
    const previousDependencies = ref<D | null>(null);

    const areDependenciesEqual = (a: D | null, b: D): boolean => {
